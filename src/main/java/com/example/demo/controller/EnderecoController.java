...
public class EnderecoController {
    ...
    @GetMapping
    private List<Endereco> buscarEndereco(){
        return enderecoService.findAllEndereco();
    }


    @PostMapping("/{id}")
    private Endereco salvarEndereco(@PathVariable Long id, @RequestBody Endereco endereco){
        return enderecoService.saveEndereco(id, endereco);
    }


    @DeleteMapping("/{id}")
    private void excluirEndereco(@PathVariable Long id) {
